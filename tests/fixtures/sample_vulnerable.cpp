...
public:
    ...
    // ECDSA signature
    int sign_certificate(const unsigned char* msg, size_t len, unsigned char* sig, EC_KEY* key)
    {
...
public:
    ...
    void sign_data(const unsigned char* hash, unsigned int hlen, unsigned char* sig,
                   unsigned int* slen)
    {
...
DH* create_dh_params()
{
    ...
    DH_generate_parameters_ex(dh, 2048, DH_GENERATOR_2,
                              nullptr);  // VULN [FN-DB]: DH_generate_parameters_ex
    return dh;
}
...
void evp_verify(EVP_PKEY* key, const unsigned char* data, size_t len, const unsigned char* sig,
                size_t sig_len)
{
    EVP_MD_CTX* mdctx = EVP_MD_CTX_new();
...
int sign_with_dsa(DSA* dsa, const unsigned char* dgst, int dlen, unsigned char* sig,
                  unsigned int* siglen)
{
...
void manual_mod_exp(BIGNUM* r, const BIGNUM* a, const BIGNUM* p, const BIGNUM* m, BN_CTX* ctx)
{
