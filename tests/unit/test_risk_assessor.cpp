                                 ...
                                 const std::string& file = "src/net.cpp", int line = 10)
{
    ...
    f.algorithm = alg;
    f.category = cat;
    f.base_risk_score = score;
    f.file_path = file;
    f.line_number = line;
    f.quantum_vulnerability = "high";
    return f;
}
int run_risk_assessor_tests()
{
    ...
    ts.add("network_file_increases_risk", [&]() {
        ...
        auto f = make_finding("RSA_generate_key_ex", "openssl", "RSA", "asymmetric_key_generation",
                              9.5, "src/network/tls.cpp");
    ...
    ts.add("migration_order_sorted_by_risk", [&]() {
        pqc::RiskAssessor ra(db);
        pqc::ProjectInventory inv;
        pqc::Finding f1 = make_finding("RSA_generate_key_ex", "openssl", "RSA",
                                       "asymmetric_key_generation", 9.5, "src/net.cpp");
        pqc::Finding f2 =
            make_finding("SHA1", "openssl", "SHA-1", "hash_function", 6.5, "src/util.cpp");
    ...
    ts.add("file_summaries_populated", [&]() {
        ...
        pqc::ProjectInventory inv;
        auto f =
            make_finding("RSA_generate_key_ex", "openssl", "RSA", "asymmetric_key_generation", 9.5);
        auto rr = ra.assess({f}, inv);
