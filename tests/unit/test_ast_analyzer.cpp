...
#ifdef PQC_TEST_OPENSSL_INCLUDE_DIR
static void configure_ast_for_tests(pqc::ASTAnalyzer& aa)
{
    aa.set_include_dirs({PQC_TEST_OPENSSL_INCLUDE_DIR});
}
#else
static void configure_ast_for_tests(pqc::ASTAnalyzer&) {}
#endif
...
int run_ast_analyzer_tests()
{
    ...
    ts.add("no_system_header_findings", []() {
        ...
        configure_ast_for_tests(aa);
        auto findings = aa.analyze_file(PQC_TEST_FIXTURES_DIR "/sample_vulnerable.cpp");
        for (auto& f : findings) ASSERT_CONTAINS(f.file_path, "sample_vulnerable");
    });
