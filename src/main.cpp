...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        if(cfg.verbose) std::cout << "[1/6] Source: " << cfg.source_path << "\n";
        ...
        if(cfg.verbose)
            std::cout << "[2/6] DB loaded: " << db.size() << " functions (v"
                      << db.db_version() << ", updated " << db.last_updated() << ")\n";

        // ── Step 3: Project inventory ─────────────────────────────────────
        pqc::ProjectScanner scanner;
        pqc::ProjectInventory inv = scanner.scan(cfg.source_path);
        if(cfg.verbose) inv.print_summary();
        auto source_files = inv.get_source_files();
        if(cfg.verbose)
            std::cout << "[3/6] Inventory: " << inv.files.size()
                      << " total files, " << source_files.size() << " source/header\n";
        ...
        if(cfg.analysis_mode == pqc::AnalysisMode::STATIC_AST) {
            ...
            if(cfg.verbose) std::cout << "[4/6] Regex analysis\n";
        ...
        if(!cfg.skip_cert && !cfg.cert_path.empty()) {
            if(cfg.verbose) std::cout << "[5/6] Certificate analysis: " << cfg.cert_path << "\n";
            pqc::CertAnalyzer cert_analyzer;
            cert_infos = cert_analyzer.analyze(cfg.cert_path);
            if(cfg.verbose) std::cout << "      Analyzed " << cert_infos.size() << " certificates\n";
        } else {
            if(cfg.verbose) std::cout << "[5/6] Certificate analysis: skipped\n";
        }

        // ── Step 6: Risk assessment + migration plan ──────────────────────
        if(cfg.verbose) std::cout << "[6/6] Risk assessment & migration plan\n";
