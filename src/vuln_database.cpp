...
void VulnDatabase::load(const std::string& path)
{
    ...
    if (!ifs.is_open())
        throw std::runtime_error("Cannot open vulnerability database: " + path);
    nlohmann::json j;
    ifs >> j;
    merge_json(j);
}
...
{
    if (j.contains("version") && version_.empty())
        version_ = j["version"].get<std::string>();
    if (j.contains("last_updated") && last_updated_.empty())
        last_updated_ = j["last_updated"].get<std::string>();
    ...
    for (auto it = libs.begin(); it != libs.end(); ++it) {
        ...
        for (auto& fn : fns) {
            ...
            if (fn.contains("aliases"))
                for (auto& a : fn["aliases"]) vf.aliases.push_back(a.get<std::string>());
            if (fn.contains("patterns"))
                for (auto& p : fn["patterns"]) vf.patterns.push_back(p.get<std::string>());
            ...
            if (fn.contains("replacements")) {
                for (auto& rep : fn["replacements"]) {
                    ...
                    ri.migration_notes = rep.contains("migration_notes")
                                             ? rep.value("migration_notes", "")
                                             : rep.value("notes", "");
