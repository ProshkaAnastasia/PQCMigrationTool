...
{
    if (cat == "asymmetric_key_generation")
        return "keyGeneration";
    if (cat == "asymmetric_encryption")
        return "keyEncapsulation";
    if (cat == "key_exchange")
        return "keyAgreement";
    if (cat == "digital_signature")
        return "sign";
    if (cat == "hash_function")
        return "digest";
    if (cat == "asymmetric_primitive")
...
static int classical_sec(const std::string& alg, int /*key_bits*/)
{
    ...
    if (a.find("ec") != std::string::npos)
        return 128;
    if (a.find("sha256") != std::string::npos)
        return 128;
    if (a.find("sha1") != std::string::npos)
        return 80;
    if (a.find("md5") != std::string::npos)
        return 64;
    return 0;
}
Cbom ReportGenerator::build_cbom(const std::vector<Finding>& findings,
                                 const ProjectInventory&) const
{
...
nlohmann::json ReportGenerator::nist_compliance_section(const RiskReport& rr)
{
    ...
    j["recommended_algorithms"] =
        nlohmann::json::array({{{"standard", "FIPS 203"},
                               ...
                               {{"standard", "draft FIPS 206"},
                                ...
                                {"nist_levels", "1/5"}}});
    j["hybrid_mode_note"] =
        "During transition, use hybrid key exchange: X25519+ML-KEM-768 and ECDSA+ML-DSA-65.";
    return j;
}
nlohmann::json ReportGenerator::tc26_compliance_section(const RiskReport& rr)
{
    ...
    j["gost_vulnerabilities"] = nlohmann::json::array(
        {{{"algorithm", "ГОСТ Р 34.10-2012"},
          {"status", "vulnerable"},
          {"reason",
           "Алгоритм основан на задаче дискретного логарифмирования на эллиптических кривых "
           "(ЗАДЛП). Квантовый компьютер решает ЗАДЛП за полиномиальное время (алгоритм Шора)."},
                               ...
                               const AnalysisMetrics* metrics) const
{
    ...
    std::cout << "[OK] CBOM written: " << cfg.cbom_output_path << " (" << findings.size()
              << " components)\n";
    if (cfg.cbom_only)
        return;
                                                  ...
                                                  const AnalysisMetrics* metrics) const
{
    ...
    j["tool"] = {{"name", "pqc-migration-tool"},
                 {"version", "2.0.0"},
                 {"standards",
                  {"NIST IR 8547", "NIST FIPS 203", "NIST FIPS 204", "NIST FIPS 205", "ТК 26"}}};
    ...
    for (auto& f : findings) {
        fa.push_back({{"file_path", f.file_path},
                      {"line", f.line_number},
                      {"col", f.column},
                      ...
                      {"category", f.category},
                      {"quantum_vulnerability", f.quantum_vulnerability},
                      {"base_risk_score", f.base_risk_score},
                      {"analyzer_mode", f.analyzer_mode},
                      {"context_function", f.context_function},
                      {"context_class", f.context_class},
    ...
    j["findings"] = fa;
    j["findings_count"] = (int)findings.size();
    nlohmann::json ca;
    ca["count"] = (int)certs.size();
    nlohmann::json cv = nlohmann::json::array();
    for (auto& c : certs) cv.push_back(c.to_json());
    ca["certificates"] = cv;
    ...
    if (metrics)
        j["analysis_metrics"] = metrics->to_json();
    return j;
}
}  // namespace pqc
