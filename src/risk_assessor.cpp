...
std::string priority_str(MigrationPriority p)
{
    switch (p) {
        case MigrationPriority::CRITICAL:
            return "critical";
        case MigrationPriority::HIGH:
            return "high";
        case MigrationPriority::MEDIUM:
            return "medium";
        default:
            return "low";
    }
...
{
    static const std::vector<std::string> kw = {"socket", "network", "tls",       "ssl", "http",
                                                "client", "server",  "conn",      "tcp", "udp",
                                                "net",    "proto",   "transport", "rpc"};
    std::string lp = p;
    std::transform(lp.begin(), lp.end(), lp.begin(), ::tolower);
    for (auto& k : kw)
        if (lp.find(k) != std::string::npos)
            return true;
    return false;
}
...
{
    static const std::vector<std::string> kw = {"store",   "storage", "db",     "database",
                                                "persist", "save",    "file",   "disk",
                                                "cache",   "repo",    "serial", "archive"};
    std::string lp = p;
    std::transform(lp.begin(), lp.end(), lp.begin(), ::tolower);
    for (auto& k : kw)
        if (lp.find(k) != std::string::npos)
...
int RiskAssessor::estimate_frequency(const Finding& f, const ProjectInventory& inv)
{
    ...
    for (auto* fe : inv.get_source_files()) {
        if (fe->path.string() == f.file_path)
            freq += fe->line_count / 300;
    }
...
ContextFactors RiskAssessor::analyze_context(const Finding& f, const ProjectInventory& inv) const
{
    ...
    if (ctx.is_key_material)
        ...
        ctx.data_classification = "long_term_data";
    else if (ctx.is_network_facing)
        ctx.data_classification = "session";
    else
        ctx.data_classification = "arbitrary";
    return ctx;
}
RiskScore RiskAssessor::compute_risk(const Finding& f, const ContextFactors& ctx) const
{
    ...
    if (ctx.is_network_facing) {
        mult *= 1.3;
        oss << " NetworkFacing(+30%).";
    }
    if (ctx.is_persistent_data) {
        mult *= 1.25;
        oss << " PersistentData(+25%).";
    }
    if (ctx.is_key_material) {
        mult *= 1.2;
        oss << " KeyMaterial(+20%).";
    ...
    if (rs.final_score >= 9.0)
        rs.priority = MigrationPriority::CRITICAL;
    else if (rs.final_score >= 7.0)
        rs.priority = MigrationPriority::HIGH;
    else if (rs.final_score >= 5.0)
        rs.priority = MigrationPriority::MEDIUM;
    else
        rs.priority = MigrationPriority::LOW;
    rs.factors = ctx;
    return rs;
}
MigrationAction RiskAssessor::build_action(const Finding& f, const RiskScore& rs, int order) const
{
    ...
    if (opt) {
        ...
        if (a.tc26_note.empty())
            a.tc26_note = opt->tc26_reference;
    }
    if (opt && !opt->replacements.empty()) {
        ...
        if (a.tc26_note.empty() && !rep.tc26_note.empty())
            a.tc26_note = rep.tc26_note;
    }
    return a;
}
RiskReport RiskAssessor::assess(const std::vector<Finding>& findings, const ProjectInventory& inv,
                                const std::vector<CertInfo>& certs) const
{
    ...
    for (auto& a : report.migration_plan) {
        ...
        fs.avg_score =
            (fs.avg_score * (fs.finding_count - 1) + a.risk.final_score) / fs.finding_count;
        fs.function_names.push_back(a.current_function);
        if (a.risk.final_score >= 9.0)
            fs.overall_priority = MigrationPriority::CRITICAL;
    ...
    if (findings.empty() && certs.empty())
        ...
        report.nist_readiness = "not-started";
    else if (report.high_count > 0 || report.medium_count > 5)
        report.nist_readiness = "in-progress";
    else
        report.nist_readiness = "near-compliant";
    return report;
}
nlohmann::json FileRiskSummary::to_json() const
{
    ...
    j["max_risk_score"] = max_score;
    j["avg_risk_score"] = avg_score;
    j["overall_priority"] = priority_str(overall_priority);
    j["functions"] = function_names;
    return j;
}
nlohmann::json RiskReport::to_json() const
{
    ...
    for (auto& a : migration_plan) {
        ...
        aj["file_path"] = a.file_path;
        aj["line_number"] = a.line_number;
        aj["current_function"] = a.current_function;
        aj["current_library"] = a.current_library;
        aj["replacement"] = {{"function", a.replacement_function},
                             {"library", a.replacement_library},
                             {"standard", a.replacement_standard},
                             {"algorithm", a.replacement_algorithm},
                             {"type", a.replacement_type}};
        aj["example_code"] = a.example_code;
        aj["migration_notes"] = a.migration_notes;
        aj["nist_reference"] = a.nist_reference;
        aj["tc26_note"] = a.tc26_note;
        aj["risk"] = {{"base_score", a.risk.base_score},
                      {"context_multiplier", a.risk.context_multiplier},
                      {"final_score", a.risk.final_score},
