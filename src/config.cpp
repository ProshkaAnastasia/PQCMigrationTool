...
void AppConfig::print_usage(const char* prog)
{
    std::cerr
        ...
        << " -s <source_path> [options]\n\n"
           ...
           "Database:\n"
           "  -d, --db <path>              Vulnerability DB (default: "
           "data/vulnerable_functions.json)\n"
...
AppConfig AppConfig::parse(int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "-s" || a == "--source")
            ...
            cfg.output_path = nxt();
        else if (a == "--cbom-output")
            cfg.cbom_output_path = nxt();
        else if (a == "-m" || a == "--mode") {
            std::string m = nxt();
            if (m == "ast")
                cfg.analysis_mode = AnalysisMode::STATIC_AST;
            else if (m == "regex")
                cfg.analysis_mode = AnalysisMode::REGEX;
            else
                throw std::invalid_argument("Unknown mode: " + m + ". Use regex or ast");
        } else if (a == "-I" || a == "--include")
            cfg.include_dirs.push_back(nxt());
        else if (a == "--extra-db")
            cfg.extra_db_paths.push_back(nxt());
        else if (a == "--cbom-only")
            cfg.cbom_only = true;
        else if (a == "--skip-cert")
            cfg.skip_cert = true;
        else if (a == "--metrics")
            cfg.run_metrics = true;
        else if (a == "--ground-truth")
            cfg.ground_truth_path = nxt();
    ...
    if (cfg.source_path.empty())
        throw std::invalid_argument("Source path (-s) is required");
