...
namespace pqc {
enum class FileCategory { SOURCE, HEADER, BUILD, CONFIG, TEST, DOCUMENTATION, RESOURCE, CERTIFICATE, OTHER };
std::string file_category_str(FileCategory c);
struct FileEntry {
    std::filesystem::path path;
    FileCategory category=FileCategory::OTHER;
    std::string language;
    size_t size_bytes=0; int line_count=0;
};
...
public:
    ...
    bool matches(const std::filesystem::path& p) const override;
    FileCategory categorize(const std::filesystem::path&) const override { return FileCategory::BUILD; }
};
class CertFilePlugin : public ILanguagePlugin {
public:
    std::string language_name() const override { return "x509"; }
    bool matches(const std::filesystem::path& p) const override;
    FileCategory categorize(const std::filesystem::path&) const override { return FileCategory::CERTIFICATE; }
};
